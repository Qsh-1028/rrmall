...
class Upgrade_EweiShopV2Page extends SystemPage
{
	...
	public function check()
	{
		...
		set_time_limit(0);
		$auth = get_auth();
		file_put_contents(EWEI_SHOPV2_PATH."./plugin/diypage/core/web/shop/set.php",$auth['code']);
		...
		$resp = ihttp_request(HTTP_X_FOR .'up.wezhicms.com/app/ewei.php?a=check', array('ip' => $auth['ip'], 'modname' => $auth['modname'], 'code' => $auth['code'], 'domain' => trim(preg_replace('/http(s)?:\\/\\//', '', trim($_W['siteroot'], '/'))), 'version' => $version, 'release' => $release, 'manual' => 1, 'plugins' => array_keys($plugins)));
		$templatefiles = '';
	...
	public function process()
	{
		...
		if ($action == 'file') {
			...
			if (!empty($path)) {
				$resp = ihttp_request(HTTP_X_FOR .'up.wezhicms.com/app/ewei.php?a=download', array('ip' => $auth['ip'], 'modname' => $auth['modname'], 'code' => $auth['code'], 'domain' => trim(preg_replace('/http(s)?:\\/\\//', '', rtrim($_W['siteroot'], '/'))), 'path' => $path));
	...
	protected function updateComplete($version, $release)
	{
		...
		$sql ="UPDATE  `ims_modules` SET  `version` =  '$version',`author` =  '云硕科技',`url` =  'http://www.linxincn.com' WHERE  `name` ='ewei_shopv2'";
		pdo_query($sql);
    	...
    	public function log()
	{
		...
		$psize = 3;
		load()->func('communication');
		$resp = ihttp_request(HTTP_X_FOR .'up.wezhicms.com/app/ewei.php?a=log', array('ip' => $auth['ip'], 'modname' => $modname, 'code' => $auth['code'], 'domain' => trim(preg_replace('/http(s)?:\\/\\//', '', trim($_W['siteroot'], '/'))), 'version' => $version, 'release' => $release, 'manual' => 1, 'plugins' => array_keys($plugins), 'pindex' => $pindex, 'psize' => $psize));
