		...
		foreach ($list as $key => $row) {
			...
			if ($row['limitgoodcatetype'] == 0 && $row['limitgoodtype'] == 0 && $row['enough'] == 0 && $row['limitdiscounttype'] == 0) {
				...
				foreach ($goodlist as $good) {
					...
					if ($row['limitdiscounttype'] == 1) {
						if ($good['istime'] == 1) {
							if (time() < $good['timestart'] || $good['timeend'] < time()) {
								$canuse = 1;
							}
						}
						else if ($good['isdiscount'] == 1) {
							if ($good['isdiscount_time'] <= time()) {
								$canuse = 1;
							}
							else {
								$discounts = json_decode($good['isdiscount_discounts'], true);

								if (empty($discounts['default']['option' . $good['optionid']])) {
									$canuse = 1;
								}
							}
						}
					...
					else if ($row['limitdiscounttype'] == 2) {
						$discounts = json_decode($good['discounts'], true);

						if ($discounts['type'] == 0) {
							if (empty($discounts['default']) && empty($discounts['default_pay'])) {
								$canuse = 1;
							}
						}
						else {
					...
					else if ($row['limitdiscounttype'] == 3) {
						if ($good['istime'] == 1) {
							if (time() < $good['timestart'] || $good['timeend'] < time()) {
								$discounts = json_decode($good['discounts'], true);
				...
				if (0 < $row['enough'] && $enough < $row['enough'] || $canuse == 0) {
					$pass = 0;
				}
			}
